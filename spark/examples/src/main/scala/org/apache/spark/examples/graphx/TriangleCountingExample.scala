...
object TriangleCountingExample {
  def main(args: Array[String]): Unit = {
    ...
    val graph = GraphLoader.edgeListFile(sc, "hdfs://node2:9820/wcy_data/Graph2/followers-1-1y.txt", true)
      .partitionBy(PartitionStrategy.RandomVertexCut)
    // Find the triangle count for each vertex
    val triCounts = graph.triangleCount().vertices
    // Join the triangle counts with the usernames
    val users = sc.textFile("hdfs://node2:9820/wcy_data/Graph2/user-1-1kw.csv").map { line =>
