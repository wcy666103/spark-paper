...
//      .appName(s"${this.getClass.getSimpleName}")
    ...
    sc.setCheckpointDir("hdfs://node2:9820/checkpoint")
    val fllower_file = args(0)
    val user_file = args(1)

    // $example on$
    // Load the edges in canonical order and
    //
    //
    //
    // partition the graph for triangle count
    val graph = GraphLoader.edgeListFile(sc, fllower_file, true)
