...
private[spark] class Benchmark(
    ...
    resultss: mutable.Map[Long,ArrayBuffer[String]] = mutable.Map(),
    outget: Option[OutputStreamWriter] = None ){
  import Benchmark._
  ...
  def run(): Unit = {
    ...
    results.zip(benchmarks).foreach { case (result, benchmark) =>
      ...
      val roww: String = benchmark.name + "-" + "%5.0f".format(result.bestMs).trim + "-" + "%4.0f".format(result.avgMs).trim + "-" + "%5.0f".format(result.stdevMs).trim + "-" + "%10.1f".format(result.bestRate).trim + "-" + "%6.1f".format(1000 / result.bestRate).trim + "-" + "%3.1fX".format(firstBest / result.bestMs).trim
      //      val roww: String = benchmark.name + "-" + result.bestMs + "-" + result.avgMs + "-" + result.stdevMs + "-" + result.bestRate + "-" + 1000 / result.bestRate + "-" + firstBest / result.bestMs
      if (!resultss.contains(valuesPerIteration)) {
        resultss(valuesPerIteration) = ArrayBuffer()
      }
      resultss(valuesPerIteration).append(roww)
      outget.get.write(valuesPerIteration + " " + roww + '\n')
      outget.get.flush()
    }
    out.println
    // scalastyle:on
