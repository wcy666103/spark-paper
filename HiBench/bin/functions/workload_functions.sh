...
#      修改这里
#        YARN_OPTS="--executor-cores 4 --total-executor-cores 24 "
#        NEW_NAME="${CLS##*.}"
#        echo ${NEW_NAME}
#        echo ${SPARK_NAME}
        SUBMIT_CMD="${SPARK_HOME}/bin/spark-submit ${LIB_JARS} --files /opt/module/spark-3.0.0-standalone/conf/metrics.properties --properties-file ${SPARK_PROP_CONF} --class ${CLS} --master ${SPARK_MASTER} --name ${SPARK_NAME}  ${YARN_OPTS} ${SPARKBENCH_JAR} $@"
    fi
